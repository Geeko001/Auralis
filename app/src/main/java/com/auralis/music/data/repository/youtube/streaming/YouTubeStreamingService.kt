package com.auralis.music.data.repository.youtube.streaming

import android.util.LruCache
import com.auralis.music.data.SessionManager
import com.auralis.music.core.model.Song
import com.auralis.music.core.model.AppError
...
class YouTubeStreamingService @Inject constructor(
            ...
            dedupScope.async {
                try {
                    ...
                    val itQuality = if (forceLow) {
                        com.auralis.music.core.model.AudioQuality.LOW
                    } else {
    ...
    private suspend fun resolveStreamWithUrl(streamUrl: String, videoId: String, forceLow: Boolean = false): String? {
        ...
        return retryWithBackoff(opTag = "audio[$videoId@$host]") {
            ...
            if (audioQuality == com.auralis.music.core.model.AudioQuality.AUTO) {
                audioQuality = if (networkMonitor.isOnWifi()) {
                    com.auralis.music.core.model.AudioQuality.MEDIUM
                } else {
                    com.auralis.music.core.model.AudioQuality.LOW
                }
            ...
            val targetBitrate = when (audioQuality) {
                com.auralis.music.core.model.AudioQuality.LOW -> 70
                com.auralis.music.core.model.AudioQuality.MEDIUM -> 160
                com.auralis.music.core.model.AudioQuality.HIGH -> 512
    ...
    suspend fun getVideoStreamUrl(videoId: String, quality: com.auralis.music.core.model.VideoQuality? = null, forceLow: Boolean = false): String? = withContext(Dispatchers.IO) {
        getVideoStreamResult(videoId, quality, forceLow)?.videoUrl
    }
    
    suspend fun getVideoStreamResult(videoId: String, quality: com.auralis.music.core.model.VideoQuality? = null, forceLow: Boolean = false): VideoStreamResult? = withContext(Dispatchers.IO) {
        val targetQuality = if (forceLow) {
            com.auralis.music.core.model.VideoQuality.LOW
        } else {
    ...
    private suspend fun resolveVideoWithUrl(
        streamUrl: String,
        videoId: String,
        targetQuality: com.auralis.music.core.model.VideoQuality
    ): VideoStreamResult? {
        val videoCacheKey = "video_${videoId}_${targetQuality.name}"
        val audioCacheKey = "video_audio_${videoId}_${targetQuality.name}"
        val host = streamUrl.substringAfter("//").substringBefore("/")
        ...
        return retryWithBackoff(opTag = "video[$videoId@$host/$targetQuality]") {
            ...
            // Adaptive logic for AUTO quality
            if (quality == com.auralis.music.core.model.VideoQuality.AUTO) {
                quality = if (networkMonitor.isOnWifi()) {
                    com.auralis.music.core.model.VideoQuality.MEDIUM
                } else {
                    com.auralis.music.core.model.VideoQuality.LOW
                }
    ...
    fun clearCacheFor(videoId: String) {
        ...
        com.auralis.music.core.model.VideoQuality.entries.forEach { 
