...
class AuralisWidget : GlanceAppWidget() {
                ...
                LaunchedEffect(currentSong?.id) {
                    if (currentSong?.thumbnailUrl != null) {
                        try {
                            ...
                            android.util.Log.w("AuralisWidget", "Artwork load failed: ${e.message}")
                        }
                    }
                }
                    ...
                    Column(
                        ...
                        Row(
                            ...
                            Column(
                                ...
                                Text(
                                    text = currentSong?.artist ?: "Auralis Player",
                                    style = TextStyle(
...
class PlayPauseAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        val entryPoint = EntryPointAccessors.fromApplication(context, AuralisWidget.WidgetEntryPoint::class.java)
        entryPoint.getMusicPlayer().togglePlayPause()
        AuralisWidget().update(context, glanceId)
    }
}
