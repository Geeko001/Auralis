...
import com.auralis.music.core.model.AppTheme
import com.auralis.music.core.model.OutputDevice
import com.auralis.music.core.model.ThemeMode
import com.auralis.music.core.model.MiniPlayerStyle
import com.auralis.music.navigation.Destination
import com.auralis.music.navigation.NavGraph
import com.auralis.music.ui.components.ExpressiveBottomNav
import com.auralis.music.ui.components.player.ExpandablePlayerSheet
import com.auralis.music.ui.components.DominantColors
import com.auralis.music.ui.components.rememberDominantColors
import com.auralis.music.core.model.ArtistCreditInfo
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.lerp
import androidx.compose.ui.graphics.luminance
import com.auralis.music.ui.screens.player.components.VolumeIndicator
import com.auralis.music.ui.screens.player.components.SystemVolumeObserver
import com.auralis.music.ui.theme.AuralisTheme
import com.auralis.music.ui.viewmodel.PlayerViewModel
import com.auralis.music.ui.viewmodel.MainViewModel
import com.auralis.music.updater.UpdateViewModel
import com.auralis.music.updater.UpdateDialog
import com.auralis.music.updater.UpdateState
...
import com.auralis.music.pip.PipHelper
...
import com.auralis.music.ui.utils.rememberDeviceFormFactor
...
class MainActivity : ComponentActivity() {
    ...
    @Inject
    lateinit var youTubeRepository: com.auralis.music.data.repository.YouTubeRepository
    
    @Inject
    lateinit var downloadRepository: com.auralis.music.data.repository.DownloadRepository
    
    @Inject
    lateinit var musicPlayer: com.auralis.music.player.MusicPlayer
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        com.auralis.music.util.AppLog.i("MainActivity") { "onCreate started" }
        ...
        enableEdgeToEdge()
        com.auralis.music.util.SnackbarUtil.setRootView(findViewById(android.R.id.content))
        ...
        lifecycleScope.launch {
            ...
            updateViewModel.checkForUpdate(
                com.auralis.music.BuildConfig.VERSION_CODE,
                isNightly = channel == com.auralis.music.core.model.UpdateChannel.NIGHTLY
            )
            ...
            if (pendingCode != null && pendingCode > com.auralis.music.BuildConfig.VERSION_CODE) {
                ...
                updateViewModel.triggerUpdateAvailable(pendingCode, pendingName, com.auralis.music.BuildConfig.VERSION_CODE)
            }
        }
    ...
    private fun requestPermissions() {
        val missingPermissions = com.auralis.music.util.PermissionUtils.getMissingPermissions(this)
    ...
    private fun applyVariantSplashTheme() {
        ...
        val themeRes = when {
            variantName.startsWith("PULSE") -> R.style.Theme_Auralis_SplashScreen_Pulse
            variantName.startsWith("RESONANCE") -> R.style.Theme_Auralis_SplashScreen_Resonance
            variantName.startsWith("AETHER") -> R.style.Theme_Auralis_SplashScreen_Aether
            variantName == "CLASSIC" -> R.style.Theme_Auralis_SplashScreen
            // Fresh install / unknown value — match the in-app PULSE default.
            else -> R.style.Theme_Auralis_SplashScreen_Pulse
        }
        setTheme(themeRes)
    }
...
fun SuvMusicApp(
    ...
    updateViewModel: com.auralis.music.updater.UpdateViewModel,
    ...
    val playlistManagementViewModel: com.auralis.music.ui.viewmodel.PlaylistManagementViewModel = koinViewModel()
    ...
    val homeViewModel: com.auralis.music.ui.viewmodel.HomeViewModel = koinViewModel()
    val searchViewModel: com.auralis.music.ui.viewmodel.SearchViewModel = koinViewModel()

    LaunchedEffect(Unit) {
        homeViewModel.events.collect { event ->
            if (event is com.auralis.music.ui.viewmodel.HomeEvent.ShowAddToPlaylistSheet) {
    ...
    LaunchedEffect(Unit) {
        searchViewModel.events.collect { event ->
            if (event is com.auralis.music.ui.viewmodel.SearchEvent.ShowAddToPlaylistSheet) {
                playlistManagementViewModel.showAddToPlaylistSheet(event.song)
            }
        }
    }
    ...
    LaunchedEffect(playlistManagementUiState.successMessage, playlistManagementUiState.errorMessage) {
        playlistManagementUiState.successMessage?.let {
            com.auralis.music.util.SnackbarUtil.showMessage(it)
            playlistManagementViewModel.clearMessages()
        }
        playlistManagementUiState.errorMessage?.let {
            com.auralis.music.util.SnackbarUtil.showError(it)
            playlistManagementViewModel.clearMessages()
        }
    }
    ...
    val playbackInfo by playerViewModel.playbackInfo.collectAsStateWithLifecycle(initialValue = com.auralis.music.core.model.PlayerState())
    ...
    val selectedLyricsProvider by playerViewModel.selectedLyricsProvider.collectAsStateWithLifecycle(initialValue = com.auralis.music.providers.lyrics.LyricsProviderType.AUTO)
    ...
    LaunchedEffect(eventsLifecycleOwner) {
        eventsLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
            ...
            when (event) {
                is com.auralis.music.ui.viewmodel.MainEvent.PlayFromDeepLink -> {
                    playerViewModel.playFromDeepLink(event.videoId)
                }
                is com.auralis.music.ui.viewmodel.MainEvent.PlayFromLocalUri -> {
                    playerViewModel.playFromLocalUri(context, event.uri)
                }
                is com.auralis.music.ui.viewmodel.MainEvent.NavigateToAlbum -> {
                    navController.navigate(
                        com.auralis.music.navigation.Destination.Album(
                            albumId = event.browseId, name = null, thumbnailUrl = null
                        )
                    )
                }
                is com.auralis.music.ui.viewmodel.MainEvent.NavigateToPlaylist -> {
                    navController.navigate(
                        com.auralis.music.navigation.Destination.Playlist(
                            playlistId = event.playlistId, name = null, thumbnailUrl = null
                        )
                    )
                }
                is com.auralis.music.ui.viewmodel.MainEvent.NavigateToArtist -> {
                    navController.navigate(com.auralis.music.navigation.Destination.Artist(event.channelId))
                }
                is com.auralis.music.ui.viewmodel.MainEvent.NavigateToSearch -> {
                    navController.navigate(com.auralis.music.navigation.Destination.Search)
                    // TODO: pre-fill search with event.query once Search screen exposes a seed param.
                }
                is com.auralis.music.ui.viewmodel.MainEvent.ShowToast -> {
                    com.auralis.music.util.SnackbarUtil.showMessage(event.message)
                }
            }
            }
        }
    ...
    // Sleep Timer
    val sleepTimerOption by playerViewModel.sleepTimerOption.collectAsStateWithLifecycle(initialValue = com.auralis.music.player.SleepTimerOption.OFF)
    ...
    LaunchedEffect(Unit) {
        val currentVersion = com.auralis.music.BuildConfig.VERSION_CODE
    ...
    if (showWelcomeDialog) {
        com.auralis.music.ui.components.WelcomeOnboardingDialog(
            onLoginClick = {
    ...
    if (showWhatsNew) {
        com.auralis.music.ui.components.WhatsNewDialog(
            versionLabel = "Version ${com.auralis.music.BuildConfig.VERSION_NAME}",
            onDismiss = {
                showWhatsNew = false
                scope.launch {
                    sessionManager.setWhatsNewSeenVersion(com.auralis.music.BuildConfig.VERSION_CODE)
                }
            }
        )
    }
                    ...
                    if (showBottomNav && !formFactor.isPhoneLike) {
                        when {
                            formFactor == DeviceFormFactor.TV -> {
                                com.auralis.music.ui.components.TvNavigationRail(
                                    currentDestination = currentDestination,
                                    onDestinationChange = { dest ->
                                        navController.navigate(dest) {
                                            popUpTo<Destination.Home> {
                            ...
                            formFactor.isTabletLike -> {
                                com.auralis.music.ui.components.AdaptiveNavigationRail(
                                    currentDestination = currentDestination,
                                    onDestinationChange = { dest ->
                                        navController.navigate(dest) {
                                            popUpTo<Destination.Home> {
    ...
    if (showMiniPlayer) {
        ...
        ExpandablePlayerSheet(
            ...
            expandedContent = { onCollapse ->
                ...
                val playerState by playerViewModel.playerState.collectAsStateWithLifecycle(initialValue = com.auralis.music.core.model.PlayerState())
                val playerScreenState = com.auralis.music.ui.screens.player.PlayerScreenState(
                    playbackInfo = playbackInfo,
                ...
                val playerScreenActions = com.auralis.music.ui.screens.player.PlayerScreenActions(
                    onPlayPause = { playerViewModel.togglePlayPause() },
                    onSeekTo = { playerViewModel.seekTo(it) },
                    onNext = { playerViewModel.seekToNext() },
                    onPrevious = { playerViewModel.seekToPrevious() },
                ...
                com.auralis.music.ui.screens.player.PlayerScreen(
        ...
        if (showMultipleArtistsDialog) {
            com.auralis.music.ui.components.player.MultipleArtistsDialog(
        ...
        if (playlistManagementUiState.selectedSongs.isNotEmpty()) {
            com.auralis.music.ui.components.AddToPlaylistSheet(
            ...
            com.auralis.music.ui.components.CreatePlaylistDialog(
